...
var imageData = {};
var imageDataKeys = [];
selectedBrands = 'coorslight|chilling|relaxing|unplug';
...
var atlasCounts = { '32px': null, '64px': null, '128px': null, '256px': null }
...
function loadData() {
  get('brand_counts.json', function(data) {
    ...
    brandCounts = [
    {x: "coorslight", value: brandCountsHolder["GreatTV"]},
    {x: "chilling", value: brandCountsHolder["chilling"]},
    {x: "relaxing", value: brandCountsHolder["relaxing"]},
    {x: "unplug", value: brandCountsHolder["unplug"]}
];
  })    
  get('brand_clusters.json', function(data) {
    clusterData = JSON.parse(data);
  }) 
  get('image_to_brand.json', function(data) {
    imageBrand = JSON.parse(data);
  }) 
  get('image_to_cluster.json', function(data) {
    imageCluster = JSON.parse(data);
  }) 
  ...
  get('plot_data.json', function(data_orig) {
    ...
    for(var i=0; i<dataHold.positions.length; i++){
        ...
        if (selectedBrands.toLowerCase().includes(dataHold.positions[i][0].substring(0, 3).toLowerCase())) {
...
function updateVertices(geometry, img) {
  geometry.vertices.push(
    ...
    new THREE.Vector3(
      img.pos.x + (img.width), /*previously (img.height) * 2 */
      img.pos.y,
      img.pos.z
    ),
    new THREE.Vector3(
      img.pos.x + (img.width), /*previously (img.height) * 2 */
      img.pos.y + (img.height), /*previously (img.height) * 2 */
      img.pos.z
    ),
    new THREE.Vector3(
      img.pos.x,
...
function addCanvasEventListeners() {
  ...
  canvases[1].addEventListener('mousemove', onMousemove, false)
  canvases[1].addEventListener('mousedown', onMousedown, false)
  canvases[1].addEventListener('mouseup', onMouseup, false)
}
...
function addWindowEventListeners() {
  ...
  window.addEventListener('hashchange', function(e) {
    ...
    marker.XPos = (((coords.x + 12000)/(2480 + 12000)) * 300) - (marker.Width / 2);
    marker.YPos = (193 - ((coords.y + 12144)/(-1024 + 12144)) * 193) - marker.Height;
    ...
    $("#brandname").html('Hashtag: ' + imageBrandName);
    ...
    clusterCounts = [
    {x: "coorslight", value: thisClusterData["GreatTV"]},
    {x: "chilling", value: thisClusterData["chilling"]},
    {x: "relaxing", value: thisClusterData["relaxing"]},
    {x: "unplug", value: thisClusterData["unplug"]}
    ]
      
    mapChart.data(brandCounts);
    mapChart.container('container');
    mapChart.draw();
      
    clusterChart.data(clusterCounts);
    clusterChart.container('container2');
    clusterChart.draw();
  })
}
...
mapSprite.src = "assets/images/Coors Full Map.png";
...
var mapChart = anychart.pie();
var mapLegend = mapChart.legend();
mapLegend.enabled(false);
mapChart.background().enabled(false);
mapChart.title("Overall");
var clusterChart = anychart.pie();
var clusterLegend = clusterChart.legend();
clusterLegend.enabled(false);
clusterChart.background().enabled(false);
clusterChart.title("In Cluster");
